...
//                      Collectors.toMap(Meal::getDate, Meal::getCalories, Integer::sum)
    ...
    public static Meal mapToEntity(Map<String, String[]> parameterMap) {
        String nullableId = parameterMap.get("id")[0];
        Integer id = Objects.nonNull(nullableId) && nullableId.length() > 0 ? Integer.parseInt(nullableId) : null;
        int calories = Integer.parseInt(parameterMap.get("calories")[0]);
        String description = new String(parameterMap.get("description")[0].getBytes(StandardCharsets.UTF_8));
        LocalDateTime dateTime = LocalDateTime.parse(parameterMap.get("dateTime")[0]);
